...
export function AppLayout({ children, title, subtitle, actions }: AppLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen bg-muted/40">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-border bg-background transition-transform lg:sticky lg:top-0 lg:h-screen lg:self-start lg:translate-x-0 ${
      ...
      >
        ...
        <div className="flex h-16 items-center gap-2.5 border-b border-black/10 bg-brand-orange px-5 text-white">
          ...
          <button
            ...
            className="ml-auto grid h-10 w-10 place-items-center rounded-full hover:bg-brand-orange-ink/10 lg:hidden"
            onClick={() => setSidebarOpen(false)}
          >
            <X className="h-4 w-4" />
          </button>
              ...
              <Link
                ...
                className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${
                  active
                    ...
                    : "text-muted-foreground hover:bg-brand-orange/10 hover:text-brand-orange-deep"
                }`}
              >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Top bar */}
        <header className="sticky top-0 z-10 flex min-h-16 items-center gap-3 border-b border-black/10 bg-brand-orange px-4 py-2.5 text-brand-orange-ink sm:px-6">
          <button
            aria-label="Abrir menú"
            className="grid h-11 w-11 place-items-center rounded-full border border-white/40 text-white transition-colors hover:bg-white/15 lg:hidden"
            onClick={() => setSidebarOpen(true)}
          >
            <Menu className="h-4 w-4" />
          </button>
          <div className="min-w-0 flex-1">
            <h1 className="font-display text-base font-bold leading-tight text-white sm:text-xl">{title}</h1>
            {subtitle && <p className="text-xs leading-snug text-white">{subtitle}</p>}
          </div>
